...
2. Copia tu carpeta "Agorateca26" a: `C:\xampp\htdocs\`
3. Abre tu navegador y ve a: `http://localhost/Agorateca26/` (ya no necesitas especificar `login.php`)
4. Prueba registrar un nuevo usuario en: `http://localhost/Agorateca26/register.php`

## Archivos del proyecto:

- **index.php**: Página principal después del login (antes dashboard.php).
- **login.php**: Formulario de inicio de sesión.
- **register.php**: Formulario de registro.
- **config.php**: Configuración de conexión a la base de datos.
- **login_process.php**: Procesa el inicio de sesión.
- **register_process.php**: Procesa el registro y envía correo de verificación.
- **verify.php**: Valida el token de verificación del correo.
- **registration_pending.php**: Página que informa que se ha enviado un correo.
...
✅ Restricción de registro por dominio de email

## Mensajes de error en login.php:

- `?error=empty`: Campos vacíos
- `?error=invalid`: Contraseña o email incorrectos
- `?error=notfound`: Usuario no encontrado
- `?error=not_verified`: La cuenta no ha sido verificada
...
- `?error=email_invalid_domain`: Dominio de correo no permitido
- `?error=email_exists`: El email ya está registrado
- `?error=username_exists`: El nombre de usuario ya existe
- `?error=database` o `email_send_failed`: Error al guardar o enviar correo
...
- Agregar más funcionalidades a `index.php`